...
public class StepService extends Service implements SensorEventListener {
    ...
    private void initNotification() {
        ...
        mBuilder.setContentTitle(getResources().getString(R.string.app_name))
                .setContentText("Die heutigen Schritte" + CURRENT_STEP )
    ...
    private void initTodayData() {
        ...
        if (list.size() == 0 || list.isEmpty()) {
            ...
            Log.v(TAG, "Etwas ist schief gelaufen！");
        }
        if (mStepCount != null) {
...
//                    Logger.d("重置步数" + StepDcretor.CURRENT_STEP);
    ...
    private void remindNotify() {
        ...
        mBuilder.setContentTitle("Die heutigen Schritte" + CURRENT_STEP + " Schritte")
                .setContentText( (Integer.valueOf(plan) - CURRENT_STEP) + "Schritte vom Ziel entfernt" )
                .setContentIntent(hangPendingIntent)
                .setTicker(getResources().getString(R.string.app_name) + "Erinnere dich daran, dass es Zeit ist zu trainieren")//通知首次出现在通知栏，带上升动画效果的
                .setWhen(System.currentTimeMillis())//通知产生的时间，会在通知信息里显示
                .setPriority(Notification.PRIORITY_DEFAULT)//设置该通知优先级
    ...
    private void addBasePedometerListener() {
        ...
        if (isAvailable) {
            Log.v(TAG, "Beschleunigungssensoren können verwendet werden");
        } else {
            Log.v(TAG, "Der Beschleunigungsmesser ist nicht verfügbar");
        }
    }
    ...
    /**
     * 保存记步数据
     * Speichern Sie Ihre Schrittdaten
     */
    class TimeCount extends CountDownTimer {
        ...
        public void onFinish() {
            // 如果计时器正常结束，则开始计步
            // Wenn der Timer normal endet, beginnt die Schrittzählung
            time.cancel();
            save();
            startTimeCount();
        }
